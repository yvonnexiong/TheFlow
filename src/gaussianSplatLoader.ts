...
// Constants & Types
// ------------------------------------------------------------
// The scene splats are 29–65MB. Over a phone hotspot to a headset that can
// take well past 30s, and a timeout here surfaces as "no splats" with only a
// console error to explain it.
const LOAD_TIMEOUT_MS = 180_000;

/**
 * Multiplier on Spark's platform LoD splat budget. 1.0 = platform default
 * (500-750K on WebXR). Raise for sharper splats, lower if the Quest drops
 * frames or runs out of memory. This is the main visual-quality dial.
 */
const LOD_SPLAT_SCALE = 2.5;
...
export class GaussianSplatLoaderSystem extends createSystem({
  ...
  init() {
    const spark = new SparkRenderer({
      ...
      enableLod: true,
      // WebXR gets Spark's most conservative LoD budget — 500-750K splats,
      // vs 2.5M on desktop. Two scene splats are resident at once for the
      // morph and share that budget (the dissolved one still occupies its
      // share even at alpha 0), so at scale 1.0 each world renders at roughly
      // half of an already-low target and looks blobby. This buys the detail
      // back at the cost of splat count — lower it if frame rate suffers.
      lodSplatScale: LOD_SPLAT_SCALE,
      // Don't waste fill rate on sub-pixel splats. Spark's docs note values up
      // to 5.0 are often indistinguishable; this offsets some of the cost of
  ...
  // ----------------------------------------------------------
  // Accessor – the loaded SplatMesh, for attaching dyno modifiers
  // ----------------------------------------------------------
  /** The entity's loaded `SplatMesh`, or null if it hasn't finished loading. */
  getSplat(entity: Entity): SplatMesh | null {
    return this.instances.get(entity.index)?.splat ?? null;
  }
