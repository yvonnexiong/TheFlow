    ...
    this.queries.sensaiPanel.subscribe("qualify", (entity) => {
      ...
      xrButton.addEventListener("click", () => {
        if (this.world.visibilityState.value === VisibilityState.NonImmersive) {
          Promise.resolve(this.world.launchXR()).catch((err) => {
            console.error("[Panel] launchXR() failed:", err);
          });
        } else {
          Promise.resolve(this.world.exitXR()).catch((err) => {
            console.error("[Panel] exitXR() failed:", err);
          });
        }
      });
